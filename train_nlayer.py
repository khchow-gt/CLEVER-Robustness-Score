...
def train(data, file_name, params, num_epochs=50, batch_size=256, train_temp=1, init=None, lr=0.01, decay=1e-5,
          momentum=0.9, activation="relu", optimizer_name="sgd"):
    """
...
def train_cnn_7layer(data, file_name, params, num_epochs=50, batch_size=256, train_temp=1, init=None, lr=0.01,
                     decay=1e-5, momentum=0.9, activation="relu", optimizer_name="sgd"):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="mnist", choices=["mnist", "cifar"],
                        help='model name')
    parser.add_argument('--modelfile', default="",
                        help='override the model filename, use user specied one')
    parser.add_argument('--modelpath', default="models_training",
                        help='folder for saving trained models')
    parser.add_argument('--modeltype', default="mlp", choices=["mlp", "cnn"],
                        help='model type')
    parser.add_argument('layer_parameters', nargs='+',
    ...
    if not args.modelfile:
        if args.modeltype == "mlp":
            file_name = args.modelpath + "/" + args.model + "_" + args.modeltype + "_" + str(
                nlayers) + "layer_" + args.activation + "_" + args.layer_parameters[0]
        elif args.modeltype == "cnn":
            file_name = args.modelpath + "/" + args.model + "_" + args.modeltype + "_" + str(
                nlayers) + "layer_" + args.activation
    else:
    ...
    # start training:
    if args.modeltype == "mlp":
        train(data, file_name=file_name, params=args.layer_parameters, num_epochs=args.epochs, lr=args.lr,
              decay=args.wd, activation=args.activation, optimizer_name=args.optimizer)
    elif args.modeltype == "cnn":
        train_cnn_7layer(data, file_name=file_name, params=args.layer_parameters, num_epochs=args.epochs, lr=args.lr,
                         decay=args.wd, activation=args.activation, optimizer_name=args.optimizer)
